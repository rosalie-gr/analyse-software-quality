...
class Make_users:
    def make_member():
        ...
        first_name = v.get_valid_input(v.name_check, "First name: ", False)
        if not first_name: return False

        last_name = v.get_valid_input(v.name_check, "Last name: ", False)
        if not last_name: return False

        age = v.get_valid_input(v.number_check, "Age: ", False)
        ...
        street_name = v.get_valid_input(v.name_check, "Street name: ", False)
        if not street_name: return False

        house_number = v.get_valid_input(v.number_check, "House number: ", False)
        ...
        mobile_phone = v.get_valid_input(v.phone_check, "Mobile phone: ", False)
